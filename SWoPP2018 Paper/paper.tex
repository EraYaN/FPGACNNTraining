...
Through this work, we seek to accelerate arbitrary precision deep-learning training using Field-Programmable Gate-Arrays by leveraging Intel FPGA SDK for OpenCL. We present our design decision and the future outlook of our framework, and quantitively describe its resource consumption and inferrence performance on modern FPGAs.
...
The usual FPGA work-flow consists of writing in a hardware description language, synthesizing the design and finally place and routing the desig onto the FGPA resources.
...
High-level synthesis takes a design specification in a high level language like C or in this case the modified version of C99 that is used for OpenCL kernels and creates an RTL design description for this design automatically after which the normal work-flow is resumed.
...
The present paper focuses on the IEEE-754 part of inference, where training and alternative data-types are on-going and future work. Althought we do have a posit~\cite{podobasposit} generator ready to be used, it has yet to be integrated into our flow.
...
The main goal is to simplify changing the numerical format, and thus all the calculations need to be done in that format, and an FPGA is better at accommodating that without a huge performance impact. % TODO add citation for "HUGE"
...
An array is used to store all output in separate locations, which enables the loop to be fully unrolled.
For the loops surrounding the inner most loop, serial dependencies are to be avoided whenever possible, so different loop iterations are fully data-independent.
...
Multiple blocks can theoretically be processed on the FPGA should space allow for it.
...
This loop cannot be pipelined due to a serial data-dependency, so it makes more sense execute this loop on the host CPU.
...
This means that all layers are an array of neurons and every neuron is connected to every neuron in the next layer.
...
The performance is memory-bound for these kernels. However, given our experience with alterantive numerical formats (most noticable posits~\cite{podobasposit}), we do not expect performance to be degraded when changing numerical format.
