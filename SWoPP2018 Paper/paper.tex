...
The goal of this framework is generality, in so far that when finished one can select which numerical format one wants to use and then compile the OpenCL kernel for that format. %TODO OpenCL explanation
While keeping the same API, so easier comparative research can be done on training performance using the different formats. % TODO Figure (Arthur?)
