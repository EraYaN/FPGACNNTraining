...
The goal of this framework is generality, in so far that when finished one can select which numerical format one wants to use and then compile the OpenCL kernel for that format.
While keeping the same API, so easier comparative research can be done on training performance using the different formats.
...
The main goal to make replacing the numerical format easier, and thus all the calculations need to be done in that format, and an FPGA is better at accomodating that without a huge performance impact.
...
One of the main choices with the largests effect on implementation is whenever to keep all weight matrices in memory transposed.
During inference this is very benificial, as this makes most reads sequential.
During training, using stochastic gradient descent, this can be a problem, the weight matrix should now be transposed back.
But since the artihmetic density of the training process is higher, the effect might be slightly less pronounced.
...
The first dataset used was MNIST, so \num{784} input features.
...
The data set was trained using the first \num{50000} images from the dataset.
...
To stop the calculation of extra padding lines in the matrices, the blocksize should be a multiple of most of the layer sizes.
...
This is becasue there are three block buffers and the relationship between blocksize and used block RAM is square.
...
Due to the FPGA's ability to use any numerical format that is required, the current implementation should not show the same performance degradation.
