...
\title{Towards Accelerating Deep Neural Network Training on FPGAs: Facilitating the Use of Variable Precision\\ \Large{(Unrefereed Workshop Manuscript)} \\ \large{This version is amended from it's original version submitted to SWoPP 2018}}
...
After which it runs the first delta kernel and continues on to run the backward kernel for all layers.
...
The training logic itself, requires two matrix-matrix multiplications and three more simple multiply substract block.
Due to the size requirements and the fact that the compiler will not successfully optimize the larger designs, the blocksize of the whole design is now limited to 24 and implementing a similar acceleration as above is impossible without using vendor IP.
...
Although it also increases the used area slightly because now the matrix multiplication block is distinct from the other ones.
...
Furthermore, given our experience with alternative numerical formats (most notably posits~\cite{podobasposit}), we do not expect performance to be degraded when changing numerical format.
...
The main limiting factor is the available block RAM and the ability for the compiler to runs its optimization phase without crashing.
...
Right now on a single device, make the limit to the block size rather small, in light of that, it was decided to postpone the further optimizations after different formats were added.
Those will also take a much larger area themselves, making the problem worse.
...
The FPGA gets slightly different floating point results due to rounding differences and reaches \SI{95.4}{\percent} and \SI{45.2}{\percent} respectively after the same number of epochs.
...
And that the training performance sufficient for further testing.
