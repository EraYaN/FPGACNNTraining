...
\title{Towards Accelerating Deep Neural Network Training on FPGAs: Facilitating the Use of Variable Precision\\ \Large{(Unrefereed Workshop Manuscript)} \\ \large{This version is amended from its original version submitted to SWoPP 2018}}
...
\affiliate{AIST}{AIST - Tokyo Tech Real World Big-Data Computation Open Innovation Laboratory, Meguro-ku, Tokyo 152-8550, Japan}{%National Institute of Advanced Industrial Science and Technology, Chiyoda-ku, Tokyo 100-8921 Japan}
...
Furthermore, simulating alternative numerical formats in software are incapable of using any compiler optimizations, nor can they leverage the vector instructions often crucial to reach application performance in modern processors.
...
\item Preliminary evaluation and analysis of core computation patterns for inference and training with respect to performance and resource utilization
\end{itemize}
...
\cref{sec:method} overviews our experimental methodology, and is followed by our results in \cref{sec:result}.
...
They are devices that belong to the class of fine-grained reconfigurable devices.\\
...
Once a particular configuration is selected, our framework can generate data-type oblivious training and inference code that targets Intel's OpenCL SDK for FPGAs.
...
Instead, our Python framework invokes a data-type selector, which automatically selects a low-level hardware description language (HDL) library that contains commonly used operators for the selected data-type.
...
The host runtime can take weights from Keras and write weights for use with Keras.
Currently it does not support Keras models for network design, since we only support Dense, Relu and Softmax layers.

The present paper focuses on the IEEE-754 part of inference and training, where alternative data-types are on-going and future work.
...
The forward kernels are the kernels the run to calculate the current activation and prediction values.
...
After which it runs the first delta kernel and continues on to run the backward kernel for all layers in reverse order.
For performance the last couple of layers could obviously be done on the host, given the limited size of most of those layers, but performance is not the main goal of this effort.
...
All weight matrices are transposed in memory, this to optimize the memory access pattern of the main inference matrix-matrix multiplication.
The inference is performed using an blocking implementation of the matrix multiplication.
...
But due to the space required for the training logic, on our current hardware (\cref{tab:test-hardware}) this does not fit or at least it will not pass the optimization stage on the OpenCl SDK compiler.

The training logic itself, requires two matrix-matrix multiplications and three more simple multiply and substract blocks.
Due to the size requirements and the fact that the compiler will not successfully optimize the larger designs, the blocksize of the whole design is now limited to 24 and implementing a similar acceleration as done for inference is impossible without using vendor IP.

\subsubsection{Design decisions}
One of the main choices with the largest impact on implementation is whenever to keep all weight matrices in memory transposed (Colunm Major).
...
Although it also increases the used area slightly because now the matrix multiplication block used for training is distinct from the other ones.
The transposing or indexing logic is different for each.
Another decision was move the execution of the outermost loop, the one that loops through all the layers, to the host.
...
%The final deci rant about other formats and no blocking for training.
...
``\texttt{-O3}'' is an optimization flag, configuring the compiler to enable resource-driven optimizations. % TODO verify
...
For the backward implementation the host code runs at \numrange{500}{800} GFLOPS, due to the higher arithmetic density.
...
The results for the different final FPGA block sizes are shown in \cref{tab:inference-kernel-performance} for the inference kernels.\\
\Cref{tab:kernel-performance} shows the performance of the training kernels (they include an inference kernel), here it is clear that the training kernels are limiting the overall throughput.
The backwards kernels have a lot of potential to be optimized if required, but much is dependant on the other data types and their implementation (size), so this is better done when actually implementing this flow for that data type.
Or at least at a point when support for a certain type is added to this framework.
The used FPGA device for testing has hardware support for IEEE-754 floating point numbers, so an alternative format will greatly increase the required area for the complete design.

\begin{table}[h]
  \centering
  \caption{FPGA Inference Kernel performance}\label{tab:inference-kernel-performance}
  \begin{tabular}{lSS}
    \toprule
    ...
    16$\times$16 & 4.3 & 4.4 \\ % kernel_nn_blocking_fixed2BS16_hw_2018-11-08T05.26.51.aocx Network MNIST: [28 * 28, 512, 512, 10] CIFAR: [32 * 32 * 3, 1024, 512, 512, 10]
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[h]
  \centering
  \caption{FPGA Full Kernel performance}\label{tab:kernel-performance}
  \begin{tabular}{lSS}
    \toprule
    \textbf{Block size} & \textbf{GFLOPS MNIST}  & \textbf{GFLOPS CIFAR-10}  \\
    \midrule
    24$\times$24 & 6.6 & 6.8  \\ % kernel_nn_blocking_fixed2BS24_hw_2018-11-08T05.20.20.aocx Network MNIST: [28 * 28, 512, 512, 10] CIFAR: [32 * 32 * 3, 1024, 512, 512, 10]
    16$\times$16 & 6.3 & 6.4 \\ % kernel_nn_blocking_fixed2BS16_hw_2018-11-08T05.26.51.aocx Network MNIST: [28 * 28, 512, 512, 10] CIFAR: [32 * 32 * 3, 1024, 512, 512, 10]
    \bottomrule
  \end{tabular}
\end{table}

The performance is mostly memory-bound for these kernels. 
Especially the backwards kernels seem to have a hard limit at around \numrange{6}{7} GFLOPS in their current form.
Furthermore, given our experience with alternative numerical formats (most notably posits~\cite{podobasposit}), we do not expect performance to be degraded when changing numerical format given enough resources (area).

\subsection{FPGA Resource Utilization}
The kernel resource utilization is shown in \cref{tab:fpga-inference-util} for the inference kernels only and \cref{tab:fpga-util} shows the utilization for the full implementation.
The main limiting factor is the available block RAM and the ability for the compiler to run its optimization phase without crashing.
...
The next logical size up from \num{64} would be \num{84} (\num{3}\(\times\)\num{28}) would use to much of the block RAM already for the inference alone.
This is because there are three block buffers and the relationship between block size and used block RAM is square.
With the training kernels the theoretical need for block RAMs would triple if all matrix operations were implemented in the same way.
Right now on a single device, this makes the limit to the block size rather small, in light of that, it was decided to postpone the further optimizations after different formats were added.
Those will also take a much larger area themselves as stated before, making the problem worse.

\begin{table}[h]
  \centering
  \caption{FPGA Inference Hardware utilization for different block sizes}\label{tab:fpga-inference-util}
  \begin{tabular}{lllll}
    \toprule
...
\begin{table}[h]
  \centering
  \caption{FPGA Full Hardware utilization for different block sizes}\label{tab:fpga-util}
  \begin{tabular}{lllll}
    \toprule
    \textbf{Block size} & \textbf{Logic} & \textbf{DSPs} & \textbf{RAMs} & \textbf{Fmax} \\
    \midrule    
    24$\times$24 & \SI{30}{\percent} & \SI{7}{\percent} & \SI{50}{\percent} & \SI{223.51}{\mega\hertz}  \\ % kernel_nn_blocking_fixed2BS24_hw_2018-11-08T05.20.20.aocx
    16$\times$16 & \SI{23}{\percent} & \SI{6}{\percent} & \SI{39}{\percent} & \SI{218.24}{\mega\hertz}  \\ % kernel_nn_blocking_fixed2BS16_hw_2018-11-08T05.26.51.aocx
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Inference Performance}
The MNIST network test set accuracy after training on the CPU is \SI{97.9}{\percent}.
And the CIFAR-10 accuracy after training is \SI{47.9}{\percent}.
The FPGA gets slightly different floating point results due to rounding and implementation differences -- mainly in parallelism -- and reaches \SI{96.1}{\percent} and \SI{46.2}{\percent} respectively after the same number of epochs.
...
Also that we can correctly train on the CPU and FPGA with custom implementations of stochastic gradient descent.
...
And that the training performance is sufficient for further testing.
We are currently working on the choice of numerical format to be used in both the inference and training part of our framework.
